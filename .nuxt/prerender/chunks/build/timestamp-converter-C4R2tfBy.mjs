...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(_attrs)}><div class="grid gap-4 md:grid-cols-2"><div><label class="field-label" for="timestamp-input">${ssrInterpolate(_ctx.$t("toolUi.timestamp.timestampLabel"))}</label><input id="timestamp-input"${ssrRenderAttr("value", timestampInput.value)} class="text-input" inputmode="numeric" placeholder="1704067200 or 1704067200000"><button class="btn-primary mt-3" type="button">${ssrInterpolate(_ctx.$t("toolUi.timestamp.toDate"))}</button></div><div><label class="field-label" for="date-input">${ssrInterpolate(_ctx.$t("toolUi.timestamp.dateLabel"))}</label><input id="date-input"${ssrRenderAttr("value", dateInput.value)} class="text-input" type="datetime-local"><button class="btn-primary mt-3" type="button">${ssrInterpolate(_ctx.$t("toolUi.timestamp.toTimestamp"))}</button></div></div><div class="button-row"><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("common.loadExample"))}</button><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("common.clear"))}</button></div>`);
      if (error.value) {
        _push(`<p class="status-error">${ssrInterpolate(error.value)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="mt-5"><label class="field-label" for="timestamp-output">${ssrInterpolate(_ctx.$t("common.result"))}</label><textarea id="timestamp-output" class="textarea-input" readonly>${ssrInterpolate(result.value)}</textarea><div class="button-row"><button class="btn-secondary" type="button"${ssrIncludeBooleanAttr(!result.value) ? " disabled" : ""}>${ssrInterpolate(unref(copied) ? _ctx.$t("common.copied") : _ctx.$t("common.copyResult"))}</button></div></div></div>`);
    };
  }
});
...
_sfc_main$1.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/tools/TimestampConverter.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "timestamp-converter",
  __ssrInlineRender: true,
  setup(__props) {
    const tool = getToolById("timestamp-converter");
    const { t } = useI18n();
    const howToUseKeys = [
      "pages.toolPages.timestampConverter.howToUse[0]",
      "pages.toolPages.timestampConverter.howToUse[1]"
    ];
    const faqKeys = [
      "pages.toolPages.timestampConverter.faq[0]",
      "pages.toolPages.timestampConverter.faq[1]",
      "pages.toolPages.timestampConverter.faq[2]"
    ];
    useSeo({
      title: t("pages.toolPages.timestampConverter.seoTitle"),
      description: t("pages.toolPages.timestampConverter.seoDescription"),
      path: tool.path
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<article${ssrRenderAttrs(mergeProps({ class: "page-container" }, _attrs))}><div class="content-shell"><header><h1 class="text-4xl font-bold tracking-normal text-slate-950">${ssrInterpolate(_ctx.$t("pages.toolPages.timestampConverter.h1"))}</h1><p class="mt-4 max-w-3xl text-lg leading-8 text-slate-700">${ssrInterpolate(_ctx.$t("pages.toolPages.timestampConverter.intro"))}</p></header><section class="tool-panel" aria-labelledby="timestamp-tool-heading"><h2 id="timestamp-tool-heading" class="section-title">${ssrInterpolate(_ctx.$t("pages.toolPages.timestampConverter.toolHeading"))}</h2>`);
      ...
      _push(`<!--]--></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.example"))}</h2><p>${ssrInterpolate(_ctx.$t("pages.toolPages.timestampConverter.example"))}</p></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.faq"))}</h2><div class="space-y-4"><!--[-->`);
      ...
      _push(`<!--]--></div></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.privacyNote"))}</h2><p>${ssrInterpolate(_ctx.$t("pages.toolPages.timestampConverter.privacy"))}</p></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.relatedTools"))}</h2>`);
...
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/tools/timestamp-converter.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=timestamp-converter-C4R2tfBy.mjs.map
