...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(_attrs)}><label class="field-label" for="json-input">${ssrInterpolate(_ctx.$t("toolUi.json.input"))}</label><textarea id="json-input" class="textarea-input" spellcheck="false"${ssrRenderAttr("placeholder", _ctx.$t("toolUi.json.placeholder"))}>${ssrInterpolate(input.value)}</textarea><div class="button-row"${ssrRenderAttr("aria-label", _ctx.$t("toolUi.json.actions"))}><button class="btn-primary" type="button">${ssrInterpolate(_ctx.$t("toolUi.json.format"))}</button><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("toolUi.json.minify"))}</button><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("toolUi.json.validate"))}</button><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("common.loadExample"))}</button><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("common.clear"))}</button></div>`);
      if (error.value) {
      ...
      if (message.value) {
        _push(`<p class="status-success">${ssrInterpolate(message.value)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="mt-5"><label class="field-label" for="json-output">${ssrInterpolate(_ctx.$t("common.output"))}</label><textarea id="json-output" class="textarea-input" readonly spellcheck="false"${ssrRenderAttr("placeholder", _ctx.$t("toolUi.json.outputPlaceholder"))}>${ssrInterpolate(output.value)}</textarea><div class="button-row"><button class="btn-secondary" type="button"${ssrIncludeBooleanAttr(!output.value) ? " disabled" : ""}>${ssrInterpolate(unref(copied) ? _ctx.$t("common.copied") : _ctx.$t("common.copyOutput"))}</button></div></div></div>`);
    };
  }
});
...
_sfc_main$1.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/tools/JsonFormatter.vue");
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const { t } = useI18n();
    const howToUseKeys = [
      "pages.toolPages.jsonFormatter.howToUse[0]",
      "pages.toolPages.jsonFormatter.howToUse[1]"
    ];
    const faqKeys = [
      "pages.toolPages.jsonFormatter.faq[0]",
      "pages.toolPages.jsonFormatter.faq[1]",
      "pages.toolPages.jsonFormatter.faq[2]",
      "pages.toolPages.jsonFormatter.faq[3]"
    ];
    useSeo({
      title: t("pages.toolPages.jsonFormatter.seoTitle"),
      description: t("pages.toolPages.jsonFormatter.seoDescription"),
      path: tool.path
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<article${ssrRenderAttrs(mergeProps({ class: "page-container" }, _attrs))}><div class="content-shell"><header><h1 class="text-4xl font-bold tracking-normal text-slate-950">${ssrInterpolate(_ctx.$t("pages.toolPages.jsonFormatter.h1"))}</h1><p class="mt-4 max-w-3xl text-lg leading-8 text-slate-700">${ssrInterpolate(_ctx.$t("pages.toolPages.jsonFormatter.intro"))}</p></header><section class="tool-panel" aria-labelledby="json-tool-heading"><h2 id="json-tool-heading" class="section-title">${ssrInterpolate(_ctx.$t("pages.toolPages.jsonFormatter.toolHeading"))}</h2>`);
      ...
      _push(`<!--]--></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.example"))}</h2><p>${ssrInterpolate(_ctx.$t("pages.toolPages.jsonFormatter.example"))}</p></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.faq"))}</h2><div class="space-y-4"><!--[-->`);
      ...
      _push(`<!--]--></div></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.privacyNote"))}</h2><p>${ssrInterpolate(_ctx.$t("pages.toolPages.jsonFormatter.privacy"))}</p></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.relatedTools"))}</h2>`);
...
//# sourceMappingURL=json-formatter-DdnRyntu.mjs.map
