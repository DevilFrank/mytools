...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(_attrs)}><label class="field-label" for="ua-input">${ssrInterpolate(_ctx.$t("toolUi.ua.input"))}</label><textarea id="ua-input" class="textarea-input"${ssrRenderAttr("placeholder", _ctx.$t("toolUi.ua.placeholder"))}>${ssrInterpolate(input.value)}</textarea><div class="button-row"><button class="btn-primary" type="button">${ssrInterpolate(_ctx.$t("toolUi.ua.parse"))}</button><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("common.loadExample"))}</button><button class="btn-secondary" type="button">${ssrInterpolate(_ctx.$t("common.clear"))}</button></div>`);
      if (error.value) {
        _push(`<p class="status-error">${ssrInterpolate(error.value)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="mt-5"><label class="field-label" for="ua-output">${ssrInterpolate(_ctx.$t("toolUi.ua.result"))}</label><textarea id="ua-output" class="textarea-input" readonly>${ssrInterpolate(result.value)}</textarea><div class="button-row"><button class="btn-secondary" type="button"${ssrIncludeBooleanAttr(!result.value) ? " disabled" : ""}>${ssrInterpolate(unref(copied) ? _ctx.$t("common.copied") : _ctx.$t("common.copyResult"))}</button></div></div></div>`);
    };
  }
});
...
_sfc_main$1.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/tools/UserAgentParser.vue");
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const { t } = useI18n();
    const howToUseKeys = [
      "pages.toolPages.userAgentParser.howToUse[0]",
      "pages.toolPages.userAgentParser.howToUse[1]"
    ];
    const faqKeys = [
      "pages.toolPages.userAgentParser.faq[0]",
      "pages.toolPages.userAgentParser.faq[1]",
      "pages.toolPages.userAgentParser.faq[2]"
    ];
    useSeo({
      title: t("pages.toolPages.userAgentParser.seoTitle"),
      description: t("pages.toolPages.userAgentParser.seoDescription"),
      path: tool.path
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<article${ssrRenderAttrs(mergeProps({ class: "page-container" }, _attrs))}><div class="content-shell"><header><h1 class="text-4xl font-bold tracking-normal text-slate-950">${ssrInterpolate(_ctx.$t("pages.toolPages.userAgentParser.h1"))}</h1><p class="mt-4 max-w-3xl text-lg leading-8 text-slate-700">${ssrInterpolate(_ctx.$t("pages.toolPages.userAgentParser.intro"))}</p></header><section class="tool-panel" aria-labelledby="ua-tool-heading"><h2 id="ua-tool-heading" class="section-title">${ssrInterpolate(_ctx.$t("pages.toolPages.userAgentParser.toolHeading"))}</h2>`);
      ...
      _push(`<!--]--></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.example"))}</h2><p>${ssrInterpolate(_ctx.$t("pages.toolPages.userAgentParser.example"))}</p></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.faq"))}</h2><div class="space-y-4"><!--[-->`);
      ...
      _push(`<!--]--></div></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.privacyNote"))}</h2><p>${ssrInterpolate(_ctx.$t("pages.toolPages.userAgentParser.privacy"))}</p></section><section class="prose-section"><h2 class="section-title">${ssrInterpolate(_ctx.$t("common.relatedTools"))}</h2>`);
...
//# sourceMappingURL=user-agent-parser-DsFYWs-D.mjs.map
